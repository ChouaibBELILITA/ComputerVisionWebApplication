...
class Charts extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(link).then((res) => {
      ...
      let totalcount = man + woman + unknown;
      let todaygenderdata = {
        total: totalcount,
        man: man,
        woman: woman,
      ...
      this.setState({
        todaygenderdata: todaygenderdata,
      });
    });

    /*Age */
    let range = this.state.ageRange;
    let ageData = this.state.todayagedata.total;
    ...
    this.setState({ todayagedata: { total: ageData, hours: perHour } });
  }
    ...
    let getAgeData = async (radioValue) => {
      ...
      if (radioValue == 0) {
        ...
        for (let i = 0; i < range.length - 1; i++) {
          ...
          let number = 0;
          for (let j = 0; j < res.data.length; j++) {
            perday.datasets[i].data[res.data[j].day] =
              res.data[j].person_numbers;
            number += res.data[j].person_numbers;
          }
          ageData.datasets[0].data[i] = number;
        ...
        for (let i = 0; i < range.length - 1; i++) {
          ...
          let number = 0;
          for (let j = 0; j < res.data.length; j++) {
            permonth.datasets[i].data[res.data[j].month] =
